(function () {
   ...
   const scriptUrl = new URL((scriptTag === null || scriptTag === void 0 ? void 0 : scriptTag.src) || window.location.href);
   ...
   async function storeRefCode(code) {
       try {
           ...
           const maxAge = convertToSeconds(org.cookieLifetimeValue, org.cookieLifetimeUnit);
   ...
   if (refCode) {
       storeRefCode(refCode).then((result) => {
           ...
           const isFirstClickMatch = affiliateData.attributionModel === "FIRST_CLICK" && !trackedCode;
           const isLastClickMatch = affiliateData.attributionModel === "LAST_CLICK" && trackedCode !== refCode;
           if (isFirstClickMatch || isLastClickMatch) {
               sendTrackingData(Object.assign({ ref: refCode, referrer: document.referrer, url: window.location.origin + window.location.pathname, host: window.location.hostname }, getDeviceInfo()));
               setCookie("refearnapp_affiliate_cookie", JSON.stringify(affiliateData), maxAge);
